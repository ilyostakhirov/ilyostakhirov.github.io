...
yii = (function ($) {
    ...
    function initScriptFilter() {
        ...
        var loadedScripts = $('script[src]').map(function () {
            return this.src.charAt(0) === 'default.htm' ? hostInfo + this.src : this.src;
        }).toArray();
        ...
        $.ajaxPrefilter('script', function (options, originalOptions, xhr) {
            ...
            var url = options.url.charAt(0) === 'default.htm' ? hostInfo + options.url : options.url;
            if ($.inArray(url, loadedScripts) === -1) {
                loadedScripts.push(url);
            } else {
                var isReloadable = $.inArray(url, $.map(pub.reloadableScripts, function (script) {
                        return script.charAt(0) === 'default.htm' ? hostInfo + script : script;
                    })) !== -1;
                if (!isReloadable) {
                    xhr.abort();
                }
